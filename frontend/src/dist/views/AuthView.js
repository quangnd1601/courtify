export const AuthView = {
    ...
    renderPayment: (bookingData, centerName, courtName) => {
        const formattedTotal = new Intl.NumberFormat("vi-VN").format(bookingData.total_price);
        const slotsString = bookingData.slots.map((s) => `${s.start_time}-${s.end_time}`).join(", ");
        return `
    ...
    renderProfile: (user, bookings, defaultTab = "profile") => {
        const renderBookingCard = (b) => {
            const formattedTotal = new Intl.NumberFormat("vi-VN").format(b.total_price);
            const slotsString = b.slots.map((s) => `${s.start_time}-${s.end_time}`).join(", ");
