...
const getImageUrl = (url, fallback = PLACEHOLDER_IMG) => {
    if (!url)
        return fallback;
...
const OwnerView = {
    ...
    renderCourtsList: (courts, centers) => {
        ...
        const rows = courts.length > 0
          ...
          <tr class="border-b border-outline-variant/10">
            ...
            <td class="px-4 py-3 text-sm">${typeof c.sport_center_id === "object" ? c.sport_center_id?.name || "—" : (centerMap[c.sport_center_id] || c.sport_center_id)}</td>
            <td class="px-4 py-3 text-sm">${c.booking_count}</td>
            <td class="px-4 py-3">${statusBadge(c.status)}</td>
            <td class="px-4 py-3">
              <div class="flex gap-2">
    ...
    renderCourtForm: (court, centers) => {
        ...
        const centerOptions = centers
            .map((c) => `<option value="${c._id}" ${court && (court.sport_center_id === c._id || (typeof court.sport_center_id === "object" && court.sport_center_id?._id === c._id)) ? "selected" : ""}>${c.name}</option>`)
            .join("");
        const content = `
      <div class="w-full">
