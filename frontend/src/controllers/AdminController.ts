...
export default class AdminController {
  ...
  // ==================== SIDEBAR LAYOUT ====================
  private renderLayout(activeTab: string, content: string): string {
    const menuItems = [
  ...
  async dashboard(): Promise<void> {
    ...
    try {
      const [users, centers, courts, bookings, vouchers, reviews] =
        await Promise.all([
          UserService.getAll(),
  ...
  async usersList(): Promise<void> {
    ...
    try {
        ...
        .map(
          ...
          <tr class="border-b border-slate-100 hover:bg-slate-50/50">
            ...
            <td class="px-4 py-3 text-sm">
              ${
                u.role !== "admin"
                  ? `<button class="toggle-user-status px-3 py-1.5 text-xs rounded-lg font-semibold transition ${
                      u.status === "active"
      ...
      app.innerHTML = this.renderLayout(
        "users",
        `<div class="p-8 text-center text-red-600">Lỗi tải danh sách người dùng.</div>`,
      );
    }
  }

  private initUsersEvents(users: any[]): void {
  ...
  async centersList(): Promise<void> {
    ...
    try {
      ...
      const rows = centers
        .map(
          ...
          <tr class="border-b border-slate-100 hover:bg-slate-50/50">
            ...
            <td class="px-4 py-3 text-sm text-slate-600">${typeof c.owner_id === "object" && c.owner_id !== null ? (c.owner_id as any).name : "—"}</td>
            <td class="px-4 py-3 text-sm text-slate-600">${c.address || c.location || "—"}</td>
            <td class="px-4 py-3 text-sm text-slate-600">${typeof c.sport_id === "object" && c.sport_id !== null ? (c.sport_id as any).name : "—"}</td>
            <td class="px-4 py-3 text-sm">
              <span class="px-2 py-0.5 rounded-full text-xs font-semibold ${
      ...
      app.innerHTML = this.renderLayout(
        "centers",
        `<div class="p-8 text-center text-red-600">Lỗi tải danh sách cụm sân.</div>`,
      );
    }
  }
  ...
  async courtsList(): Promise<void> {
    ...
    try {
      ...
      app.innerHTML = this.renderLayout(
        "courts",
        `<div class="p-8 text-center text-red-600">Lỗi tải danh sách sân.</div>`,
      );
    }
  }

  // ==================== BOOKINGS ====================
  async bookingsList(): Promise<void> {
    const app = this.getAppElement();
    if (!app) return;
    ...
    try {
      ...
      app.innerHTML = this.renderLayout(
        "bookings",
        `<div class="p-8 text-center text-red-600">Lỗi tải danh sách đặt sân.</div>`,
      );
    }
  }

  private initBookingsFilter(): void {
  ...
  async vouchersList(): Promise<void> {
    ...
    try {
      ...
      const rows = vouchers
        .map((v) => {
          const centerName =
            typeof v.sport_center_id === "object" && v.sport_center_id
              ? (v.sport_center_id as any).name
              : "—";
          const ownerName =
            typeof v.owner_id === "object" && v.owner_id
              ? (v.owner_id as any).name
              : "—";
          const statusCls =
            v.status === "active"
              ? "badge-active"
      ...
      app.innerHTML = this.renderLayout(
        "vouchers",
        `<div class="p-8 text-center text-red-600">Lỗi tải danh sách voucher.</div>`,
      );
    }
  }

  // ==================== REVIEWS ====================
  async reviewsList(): Promise<void> {
    const app = this.getAppElement();
    if (!app) return;
    ...
    try {
            ...
            <td class="px-4 py-3 text-sm">
              <div class="flex gap-0.5">
                ${Array.from({ length: 5 })
                  .map(
                    (_, i) =>
                      `<span class="text-base ${i < (r.rating || 0) ? "text-yellow-400" : "text-slate-300"}">★</span>`,
                  )
                  .join("")}
              </div>
            </td>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex flex-col items-center justify-center">
              ...
              <div class="flex gap-0.5 my-2">
                ${Array.from({ length: 5 })
                  .map(
                    (_, i) =>
                      `<span class="text-xl ${i < Math.round(parseFloat(avgRating)) ? "text-yellow-400" : "text-slate-300"}">★</span>`,
                  )
                  .join("")}
              </div>
      ...
      app.innerHTML = this.renderLayout(
        "reviews",
        `<div class="p-8 text-center text-red-600">Lỗi tải danh sách đánh giá.</div>`,
      );
    }
  }

  private initReviewsEvents(): void {
    const deleteButtons = document.querySelectorAll(".delete-review-btn");
    deleteButtons.forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        const reviewId = (e.currentTarget as HTMLElement).getAttribute(
          "data-id",
        );
        if (!reviewId) return;
