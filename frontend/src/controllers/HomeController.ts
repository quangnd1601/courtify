...
export default class HomeController {
  async init(): Promise<void> {
    ...
    try {
      ...
      if (mostBookedEl) mostBookedEl.innerHTML = HomeView.renderList(mostBooked);

      const newestEl = document.getElementById("newest-centers-list");
      if (newestEl) newestEl.innerHTML = HomeView.renderList(newest);

    } catch (error) {
      console.error("Lỗi tải trang chủ:", error);

      const mostBookedEl = document.getElementById("most-booked-centers-list");
      if (mostBookedEl) mostBookedEl.innerHTML = HomeView.renderListError();

      const newestEl = document.getElementById("newest-centers-list");
      if (newestEl) newestEl.innerHTML = HomeView.renderListError();
    }
  }
}
