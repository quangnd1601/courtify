...
import CourtService, { ISlot } from "../services/CourtService.js";
import UserService from "../services/UserService.js";
import { DetailView } from "../views/DetailView.js";

export default class DetailController {
  private centerId: string;
  private selectedSlots: ISlot[] = [];
  ...
  private async loadSlots(courtId: string, date: string): Promise<void> {
    ...
    try {
      const slots: ISlot[] = await CourtService.getAvailableSlots(courtId, date);
  ...
  private updatePriceBreakdown(): void {
    ...
    const total = this.selectedSlots.reduce((sum: number, s: ISlot) => sum + s.price, 0);
    ...
    if (subtotalEl) subtotalEl.textContent = count === 0 ? "—" : `${formatted}đ`;
    if (totalEl) totalEl.textContent = count === 0 ? "—" : `${formatted}đ`;
  ...
  private bindSlotClick(): void {
    const slotBtns = document.querySelectorAll<HTMLButtonElement>(".time-slot-btn:not([disabled])");
  ...
  private bindEvents(activeCourts: { _id: string; name: string }[]): void {
    // ── Sự kiện chọn sân ──────────────────────────────────────────────────
    const courtBtns = document.querySelectorAll<HTMLButtonElement>(".court-btn");
    ...
    if (dateInput) {
      dateInput.addEventListener("change", async () => {
        ...
        const activeCourtBtn = document.querySelector<HTMLButtonElement>(".court-btn.bg-primary-container");
    ...
    const bookingBtn = document.getElementById("booking-btn") as HTMLButtonElement;
    if (bookingBtn) {
