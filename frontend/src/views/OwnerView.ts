...
const bookingStatusBadge = (status: string): string => {
...
// Sidebar component
const renderSidebar = (active: string): string => {
  const menuItems = [
...
// Main layout wrapper
const renderLayout = (content: string, active: string, title: string): string => {
  return `
...
const OwnerView = {
  ...
  // ==================== CENTERS LIST ====================
  renderCentersList: (centers: ISportsCenter[]): string => {
    const rows =
      centers.length > 0
        ? centers
          .map(
  ...
  // ==================== CENTER FORM ====================
  renderCenterForm: (center: ISportsCenter | null, sports: ISport[]): string => {
    const isEdit = !!center;
    const sportOptions = sports
      .map(
        (s) =>
          `<option value="${s._id}" ${center && typeof center.sport_id === "object" && center.sport_id._id === s._id ? "selected" : ""}>${s.name}</option>`
      )
      .join("");
  ...
  // ==================== COURTS LIST ====================
  renderCourtsList: (courts: ICourt[], centers: ISportsCenter[]): string => {
  ...
  // ==================== COURT FORM ====================
  renderCourtForm: (court: ICourt | null, centers: ISportsCenter[]): string => {
    const isEdit = !!court;
  ...
  // ==================== BOOKINGS ====================
  renderBookings: (bookings: any[]): string => {
    const rows =
      bookings.length > 0
        ? bookings
          .map(
          ...
          <tr class="border-b border-outline-variant/10 booking-row" data-status="${b.booking_status}">
            ...
            <td class="px-4 py-3 text-sm">${formatDate(b.booking_date)}</td>
            <td class="px-4 py-3 text-sm">
              ${b.slots?.map((s: any) => `${s.start_time}-${s.end_time}`).join(", ") || "—"}
            </td>
            <td class="px-4 py-3 text-sm font-semibold">${formatMoney(b.total_price || 0)}đ</td>
            <td class="px-4 py-3">${bookingStatusBadge(b.booking_status)}</td>
            <td class="px-4 py-3">
  ...
  // ==================== VOUCHERS LIST ====================
  renderVouchersList: (vouchers: IVoucher[]): string => {
    const rows =
      vouchers.length > 0
        ? vouchers
          .map(
  ...
  // ==================== VOUCHER FORM ====================
  renderVoucherForm: (voucher: IVoucher | null, centers: ISportsCenter[]): string => {
    const isEdit = !!voucher;
    const centerOptions = centers
      .map(
        (c) =>
          `<option value="${c._id}" ${voucher && voucher.sport_center_id === c._id ? "selected" : ""}>${c.name}</option>`
      )
      .join("");
