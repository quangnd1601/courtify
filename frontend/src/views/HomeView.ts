...
export const HomeView = {
  ...
  // ─── Render card cụm sân ───────────────────────────────────────────────────
  renderCenterCard: (center: ISportsCenter): string => {
    const lowestPrice =
      center.pricing && center.pricing.length > 0
        ? Math.min(...center.pricing.map((p) => p.price))
        : 100000;
    const formattedPrice = new Intl.NumberFormat("vi-VN").format(lowestPrice);
    const sportName =
      typeof center.sport_id === "object" ? center.sport_id.name : "Thể thao";
  ...
  renderList: (list: ISportsCenter[]): string => {
    if (list.length === 0) {
  ...
  renderListError: (): string =>
    `<p class="text-red-600 col-span-full text-center py-md font-body-md">Lỗi kết nối Backend. Vui lòng thử lại!</p>`,
